import { EVENT_LABELS, MEASUREMENTS, MEASUREMENT_RANGES, ACTIVITIES } from '../model/clinical-record.js';
...
export class ClinicalView {
  ...
  record(model) {
    ...
    if (this.tab === 'series') {
      ...
      body += this.list(e.measurements, item => `${stamp(item.at)} · ${item.kind}: ${item.value} ${item.unit}`);
    }
