...
/* ── 1 · Árbol del commit base ────────────────────────────────────────────
   En RCE-KINE el base salía de `git worktree add <commit>`. NEXT arranca con
   historial propio y ese commit NO existe aquí, así que el árbol congelado
   vive versionado en build/checks/base/<commit>/v2 (ver su LEEME.md). Se
   copia a un temporal porque el banco se ejecuta DENTRO del árbol base y
   escribe ahí su build/. */
const BASE_FIJO = path.join(__dirname, 'base', BASE_COMMIT, 'v2');
if (!fs.existsSync(BASE_FIJO)) {
  console.log('❌ Falta el árbol congelado del commit ' + BASE_COMMIT + ' en ' + BASE_FIJO + '.');
  console.log('   Esta guardia es un A/B: sin el lado base no prueba nada. Ver build/checks/base/LEEME.md.');
  process.exit(1);
}
const baseDir = path.join(os.tmpdir(), 'rce_base_guardado_' + BASE_COMMIT);
fs.rmSync(baseDir, { recursive: true, force: true });
fs.mkdirSync(path.join(baseDir, 'v2'), { recursive: true });
for (const f of fs.readdirSync(BASE_FIJO)) fs.copyFileSync(path.join(BASE_FIJO, f), path.join(baseDir, 'v2', f));
