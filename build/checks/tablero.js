...
// El lado «viejo» del A/B. En RCE-KINE salía de `git show <commit>:v2/...`;
// NEXT arranca con historial propio y ese commit no existe aquí, así que el
// código congelado vive versionado en build/checks/base/ (ver su LEEME.md).
const leerBase = f => {
  const fijo = path.join(__dirname, 'base', BASE, 'v2', f);
  if (fs.existsSync(fijo)) return fs.readFileSync(fijo, 'utf8');
  try {
    return cp.execSync('git show ' + BASE + ':v2/' + f, { cwd: RAIZ, maxBuffer: 1 << 26, stdio: ['ignore', 'pipe', 'ignore'] }).toString();
  } catch (e) {
    // Sin el lado base no hay A/B posible: mejor decirlo que dar un verde que
    // no probó nada.
    console.log('❌ No se pudo leer ' + f + ' del commit ' + BASE + '. ' +
      'Esta guardia compara contra ese commit congelado: falta ' + fijo + '. ' +
      'Ver build/checks/base/LEEME.md.');
    process.exit(1);
  }
};
